locals {
  ...
  source_ca_path               = "/etc/step/certs/kube"
...
resource "null_resource" "step_k3s_ca" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo env STEP_TOKEN=${data.external.step_k3s_token[each.key].result.TOKEN} step ca certificate ${replace(each.value, "/", "-")} ${local.source_ca_path}/${each.value}.crt ${local.source_ca_path}/${each.value}.key --provisioner ${var.STEP_PROVISIONER_KUBE}",
